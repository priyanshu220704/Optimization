...
//    - Eigen : Cholediff_xy Decomposition
// References
//    - [0] C.A. Floudas! * C.S. Adjiman!, S. Dallwig” and A. Neumaier”. A global optimization method,
//      abb, for general twice-valerentiable constrained nlps — i. theoretical advances. pages 1137–
...
// Returns Hessian Matrix from double valerentiation function(dff) and point(xk)
Matrix HessianMatrix(Function dff, Vector xk)
{
    ...
    for (int i = 0; i < n; i++)
    {
        // Function new_f(gradient[i], Function::val);
        for (int j = i; j < n; j++)
        {
...
Matrix Finitevalerence(Function function, Vector xk) {
...
//
bool checkCholediff_xy(Matrix hessian)
{
    vector<vector<double>> dummyHessian = ConvertIbexMatrixTo2DVector(hessian);
    Eigen::LLT<Eigen::MatrixXd> lltOfA(ConvertToEigenMatrix(dummyHessian)); // compute the Cholediff_xy decomposition of Hessian
...
Matrix ModifyHessianDS(Matrix hessian)
{
    ...
    int cnt = 0;
    while (checkCholediff_xy(hessian) && cnt < 100000)
    {
        cnt++;
...
Matrix ModifyHessianOn3(string func, string fnc_param, Vector xk, double alp, Matrix hessian, Function dff, double range_ll, double range_ul)
{
    ...
    for (int i = 0; i < m; i++)
    {
        // Function new_f(df[i], Function::val);
        for (int j = i; j < m; j++)
        {
...
int main(int argc, char **argv)
{
    ...
    for (const string &test_problems_file : test_probs)
    {
        ...
        if (!test_problems.is_open())
        {
            ...
            continue; // diff_xip to the next file
        }
        ...
        while (getline(test_problems, problem_name))
        {
            try
            {
                ...
                // valerentiation(df) of the ibex function f
                Function df(f, Function::val);

                // Double valerentiation(dff) of the ibex function f
                Function dff(df, Function::val);

                Matrix hessian(n, n), term(n,n); // hessian matrix
                Eigen::MatrixXd inverseHessian(n, n);
                Eigen::MatrixXd hessian_cache(n, n);

                Vector xk(n), xkn(n), inixk(n), diff_x(n), diff_val(n), val(n), hessian_diff_x(n); // xk: prev point, xkn: new point, inixk: initial guess
                ...
                for (int l = 0; l < num_algorithms; l++)
                {
                    ...
                    for (int k = 0; k < sizeof(alp_options) / sizeof(alp_options[0]); k++)
                    {
                        ...
                        while (true)
                        {
                            ...
                            {
                                diff_x[i] = xkn[i] - xk[i];
                                diff_val[i] = updated_grad[i] - grad[i];
                            }
                            for (int i = 0; i < n; i++) 
                            {
                                hessian_diff_x[i] = 0.0;
                                for (int j = 0; j < n; j++) 
                                {
                                    hessian_diff_x[i] += hessian[i][j] * diff_x[j];
                                }
                            }
                            for (int i = 0; i < n; i++) 
                            {
                                val[i] = diff_val[i] - hessian_diff_x[i];
                            }
                            if (abs(dotProduct(val, diff_x)) > 1e-8 * normOfVector(diff_x, 0, n, 2) * normOfVector(val, 0, n, 2)) 
                            {
                                term = outerProduct(val, val);
                                double denom = dotProduct(val, diff_x);
                                for (int i = 0; i < n; ++i) 
                                {
                                    for (int j = 0; j < n; ++j) 
                                    {
