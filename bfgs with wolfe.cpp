    ...
    for (const string &test_problems_file : test_probs)
    {
        ...
        while (getline(test_problems, problem_name))
        {
            try
            {
                ...
                Matrix hessian(n,n), v1(n,n), v2(n,n), H(n,n), sk_yk(n,n), yk_sk(n,n); // hessian matrix
                ...
                Vector xk(n), xkn(n), inixk(n), g(n); // xk: prev point, xkn: new point, inixk: initial guess
                ...
                Vector grad(n), direcV(n), diff_x(n), diff_grad(n), updated_grad(n); // Gradient and Direction Vector
                double a_init = 1;
                    ...
                    for (int k = 0; k < sizeof(alp_options) / sizeof(alp_options[0]); k++)
                    {
                        ...
                        while (true)
                        {
                            ...
                            {  
                                ...
                                gfpk_new = dotProduct(gradVector(df, xkn), direcV);
                                if((func_value_xkn.lb()[0] <= result.lb()[0] + c * a * gfpk)  && (gfpk_new >= c2 * gfpk))
                                {
                                    break;
                                }                           
                                a = a * rh;
