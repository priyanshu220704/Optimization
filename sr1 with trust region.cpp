...
ibex::Matrix eigenToIbexMatrix(const Eigen::MatrixXd& eigenMat) {
    int rows = eigenMat.rows();
    int cols = eigenMat.cols();

    // Create ibex::Matrix with the same dimensions as Eigen::MatrixXd
    ibex::Matrix ibexMat(rows, cols);

    // Copy data from Eigen::MatrixXd to ibex::Matrix
    for (int i = 0; i < rows; ++i) {
        for (int j = 0; j < cols; ++j) {
            ibexMat[i][j] = eigenMat(i, j);
        }
    }
...
Vector solveQuadraticSubproblem(Vector gradient, Matrix hessian, Eigen::MatrixXd invhessian, double radius) {
    // Dogleg Method
    int n = gradient.size();
    Vector hk_gk(n), newton_step(n), cauchy_step(n), p(n), pn_pc(n);
    double gk_hk_gk = 0;
    
    for(int i = 0; i < n; i++)
    {
        hk_gk[i] = 0; newton_step[i] = 0;
        for(int j = 0; j < n; j++) 
        {
            hk_gk[i] += hessian[i][j] * gradient[j];
            newton_step[i] += -1 * invhessian(i,j) * gradient[j];  
        }
        gk_hk_gk += gradient[i] * hk_gk[i];
    }

    for(int i = 0; i < n; i++)
    {
        cauchy_step[i] = -1 * (dotProduct(gradient,gradient) / gk_hk_gk) * gradient[i];
        cauchy_step[i] = cauchy_step[i] * radius / normOfVector(cauchy_step, 0, n, 2); 
    }

    if(normOfVector(newton_step, 0, n, 2) <= radius) return newton_step;

    else if(normOfVector(cauchy_step, 0, n, 2) >= radius) return cauchy_step;
    ...
    else 
    {
        ...
        for(int i = 0; i < n; i++)
        {
            pn_pc[i] = newton_step[i] - cauchy_step[i];  
        }
        a = dotProduct(pn_pc, pn_pc);
        b = 2 * dotProduct(cauchy_step, pn_pc);
        c = dotProduct(cauchy_step, cauchy_step) - (radius * radius);
    
        t = (-b + sqrt(b * b - 4 * a * c)) / (2 * a);
        for(int i = 0; i < n; i++)
        {
            p[i] = cauchy_step[i] + t * pn_pc[i]; 
        }
        return p;
    }
}

// Approximate initial matrix by Finite difference 
    ...
    for (const string &test_problems_file : test_probs)
    {
        ...
        while (getline(test_problems, problem_name))
        {
            try
            {
                ...
                Matrix hessian(n,n),V(n,n), H(n,n), term(n,n); // hessian matrix
                ...
                Vector xk(n), xkn(n), inixk(n); // xk: prev point, xkn: new point, inixk: initial guess
                Vector fs_our(n), fs_ger(n);

                int num_algorithms = 6; // only using Gershgorin Algo
                string algorithms[num_algorithms] = {"Steepest Descent", "Gerschgorin", "Diagonal Shift", "On2 Interval hessian", "On3 Interval Hessian", "SR1"};
                double func_value_ger, func_value_on2, func_value_ds, func_value_on3, func_value_bfgs; // Function value of the four problems
                ...
                Vector grad(n), direcV(n), diff_x(n), diff_grad(n), updated_grad(n), hk_sk(n), diff(n), sk(n), yk(n); // Gradient and Direction Vector
                double a_init = 1;
                double a = a_init; // a: alpha to be modified with the wolfe conditions
                double sk_hk_sk, grad_sk, ared, pred, radius = 0.5;
                ...
                for (int l = 0; l < num_algorithms; l++)
                {
                    ...
                    for (int k = 0; k < sizeof(alp_options) / sizeof(alp_options[0]); k++)
                    {
                        ...
                        while (true)
                        {
                            ...
                            sk = solveQuadraticSubproblem(grad, hessian, inverseHessian, radius);
                            ...
                            updated_grad = gradVector(df, xkn); // gradient vector at pt. xkn

                            for (int i = 0; i < n; i++) 
                            {
                                yk[i] = updated_grad[i] - grad[i];
                            }
                            double rho = 0.0;
                            ...
                            ared = result.lb()[0] - func_value_xkn.lb()[0];
                            ...
                            for (int i = 0; i < n; i++) 
                            {
                                ...
                                for (int j = 0; j < n; j++) 
                                {
                                    hk_sk[i] += hessian[i][j] * sk[j];
                                }
                            }
                            
                            sk_hk_sk = dotProduct(sk, hk_sk);    
                            grad_sk = dotProduct(grad, sk);   

                            pred = -1 * grad_sk - 0.5 * sk_hk_sk;
                            ...
                            if (ared / pred > 1e-3) 
                            {
                                ...
                                updated_grad = gradVector(df, xkn);
                            }

                            if (ared / pred > 0.75) 
                            {
                                if (normOfVector(sk, 0, n, 2) <= 0.8 * radius) 
                                {
                                    radius = radius;
                                } else 
                                {
                                    radius = min(2 * radius, delta_not);
                                }
                            } 
                            else if (0.1 <= ared / pred && ared / pred <= 0.75) 
                            {
                                radius = radius;
                            } 
                            else 
                            ...
                            if (abs(dotProduct(diff, sk)) > 1e-8 * normOfVector(sk, 0, n, 2) * normOfVector(diff, 0, n, 2)) 
                            {
                                term = outerProduct(diff, diff);
                                double denom = dotProduct(diff, sk);
                                for (int i = 0; i < n; ++i) 
                                {
                                    for (int j = 0; j < n; ++j) 
                                    {
                                        hessian[i][j] += term[i][j] / denom;
                                    }
                                }
                            }
